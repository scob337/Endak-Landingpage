...
const NavBar = () => {
  ...
  return (
    <nav className="bg-white mb-10 dark:bg-gray-900  sticky  w-full
     z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
          <img loading="lazy"  src={Logo} className="w-[150px] h-[70px] object-cover" alt="Logo" />
        </Link>
          ...
          <ul className="font-medium flex flex-col items-end p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            {Links.map((link, index) => (
              <li key={index}>
                <Link
                  to={link.href}
                  className={`block py-2 px-3 rounded-sm hover:text-[#00be3c] transition-all ${isActive(link.href) ? 'text-[#00be3c]' : ''}`}
                >
                  {link.name}
                </Link>
              </li>
            ...
            {Links.map((link, index) => (
              <li key={index}>
                <Link
                  ...
                  className={`block
                    font-semibold
                    rounded-sm hover:text-[#00be3c] transition-all ${isActive(link.href) ? 'text-[#00be3c]' : ''}`}
                >
                  {link.name}
                </Link>
              </li>
